/**
 * AddSecondMemory — Step 2 of onboarding memory collection
 * Route: /persona/:id/second-memory
 * After any ONE memory added → /persona/:id/echo-intro
 */
import { useParams, Link } from "wouter";
import { useQuery } from "@tanstack/react-query";
import { Layout } from "@/components/Layout";
import { ArrowRight, Mail, BookOpen, Camera, Mic, MessageSquare, Heart } from "lucide-react";
import type { Persona } from "@shared/schema";
import { cn } from "@/lib/utils";

const API_BASE = "__PORT_5000__".startsWith("__") ? "" : "__PORT_5000__";

export default function AddSecondMemory() {
  ...
  const returnPath = `/persona/${personaId}/echo-intro`;
  ...
  const memories = [
    ...
    },
    {
      ...
      label: "Tell a story",
      description: isSelf
        ? "A different moment this time — anything that matters."
        : `Something else about ${firstName}. Any moment worth keeping.`,
    ...
    },
    {
      ...
      description: isSelf ? "Speak freely — anything at all." : `A voice recording, short or long.`,
    ...
    },
    {
      ...
      description: isSelf ? "A different question this time." : `A different question about ${firstName}.`,
  ...
  return (
    <Layout backTo={`/persona/${personaId}/first-memory`} backLabel="Back">
      <div className="max-w-2xl mx-auto px-4 sm:px-6 py-10 space-y-8">
        ...
        <div className="text-center space-y-3">
          ...
          <p className="text-sm text-muted-foreground leading-relaxed max-w-sm mx-auto">
            {isSelf
              ? "Add one more memory and something special becomes available."
              : `Add one more memory for ${firstName} and something special becomes available.`}
          </p>
          <div className="flex items-center justify-center gap-2 pt-1">
            <div className="h-1.5 w-8 rounded-full bg-primary" />
            <div className="h-1.5 w-8 rounded-full bg-primary" />
          </div>
          <p className="text-xs text-muted-foreground">Second memory of two</p>
        </div>
        ...
        <p className="text-xs text-muted-foreground text-center">
          After this, you'll be able to explore everything Echo Me has to offer.
        </p>
      </div>
    </Layout>
  );
